...
export class UsersSecurityState {
  ...
  @Selector()
  static IsUpdating(state: IUsersSecurityStateModel): boolean {
  ...
  @Action(UserSecurityUpdateSecurity)
  onUpdateSecurity(ctx: StateContext<IUsersSecurityStateModel>, action: UserSecurityUpdateSecurity) {

    const { request } = action;
    const { admin, blogger, editor } = request;

    ctx.patchState({ updating: true });
    return from(this.userSecurity.update(request.Id, { admin, blogger, editor } as IUserSecurityFirebaseModel)).pipe(
      delay(1000),
      tap(() => {
        ctx.patchState({ updating: false });
        this.snackBarStatus.OpenComplete('Security Updated');
      })
    )
  }
